...
<script setup>
    ...
    const chanTotal = ref(0);
    const todayRegUser = ref(0);
    const todayChatTotal = ref(0);
    const loadingStatus = ref(true);
    const systemOsInfo = reactive({
        osInfo: null,
        osArch: null,
        locale: null,
        hostName: null,
    ...
    const useCommonTransition = (source, duration) => {
        return useTransition(source, { duration });
    }
    
    const applicationStore = utils.useApplicationStore();
    const GET_ADMIN_DASH_DATA_EVENT = "[GET:ADMIN:DASH:DATA]";
    const userTotalValue = useCommonTransition(userTotal, 1000);
    const chanTotalValue = useCommonTransition(chanTotal, 1000);
    const todayRegUserValue = useCommonTransition(todayRegUser, 1000);
    const todayChatTotalValue = useCommonTransition(todayChatTotal, 1000);
    
    onMounted(async () => {
        applicationStore.socketIo.emit(GET_ADMIN_DASH_DATA_EVENT, null, (response) => {
        ...
        <div class="info-container">
            <div class="info-card">
                <div class="card-overview">
                    <div class="info-line">主机名称：{{ systemOsInfo.hostName }}</div>
                    <div class="info-line">系统语言：{{ systemOsInfo.locale }}</div>
                    <div class="info-line">系统架构：{{ systemOsInfo.osArch }}</div>
                    <div class="info-line">主机地址：{{ systemOsInfo.hostAddress }}</div>
                    <div class="info-line">系统时区：{{ systemOsInfo.timeZoneId }}</div>
                    <div class="info-line">运行时长：{{ systemOsInfo.systemUptime }}</div>
                    <div class="info-line">操作系统：{{ systemOsInfo.osInfo }}</div>
                </div>
            </div>

            <div class="info-card">
                <div class="card-update">
                    <div class="info-line">Handsock v2.0.1更新日志如下：</div>
                    <p>重构了整个用户系统和消息系统，新增账号登录系统、频道系统、公告系统、前端嵌入式后台管理系统，支持发送文本、图片、文件和表情等消息，移除了只能使用QQ号作为头像的弊端，移除了使用消息码作为消息类型的弊端，使用默认或用户上传作为头像，移除了用户加入频道时的消息，前端移除了部分无用的包并重构了整体的UI，后端和通信端全部由Java进行重构。</p>
                </div>
            </div>
        </div>

        <div class="footer-container">
            <p class="copyright">Copyright © 20123 - 2024 All Rights Reserved Handsock</p>
        </div>
    </div>
</template>

<style>
    @import url("./styles/DashManner.css");
</style>