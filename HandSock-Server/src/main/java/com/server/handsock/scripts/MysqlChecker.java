...
public class MysqlChecker {
    ...
    private final String FORUM_TABLE_NAME = "handsock_forum";
    ...
    private final String COMMENT_TABLE_NAME = "handsock_comment";
    ...
    private final String CREATE_USER_TABLE_SQL = """
                CREATE TABLE IF NOT EXISTS %s (
                    ...
                    `qq_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    ...
    private final String CREATE_COMMENT_TABLE_SQL = """
                CREATE TABLE IF NOT EXISTS %s (
                    ...
                    INDEX `idx_pid` (`pid`),
                    INDEX `idx_uid` (`uid`),
                    FOREIGN KEY (pid) REFERENCES %s(pid) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (uid) REFERENCES %s(uid) ON DELETE CASCADE ON UPDATE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
            """.formatted(COMMENT_TABLE_NAME, FORUM_TABLE_NAME, USER_TABLE_NAME);

    private final String CREATE_FORUM_TABLE_SQL = """
                CREATE TABLE IF NOT EXISTS %s (
            ...
            """.formatted(FORUM_TABLE_NAME, USER_TABLE_NAME);
    ...
    private final String CREATE_MESSAGE_TABLE_SQL = """
            ...
            """.formatted(MESSAGE_TABLE_NAME, USER_TABLE_NAME, CHANNEL_TABLE_NAME);
    ...
    private final String INSERT_USER_SQL = """
                INSERT INTO %s (`uid`, `nick`, `qq_id`, `status`, `avatar`, `ai_auth`, `reg_time`, `username`, `password`, `permission`) VALUES
                    (2400442120, '管理员账号', NULL, 0, '0/default/0.png', 1, CURRENT_TIMESTAMP, 'handsock', 'd0775d11a4c67b9e8df44c336fac3371', 1),
                    (2400408600, '机器人账号', NULL, 0, '0/default/1.png', 0, CURRENT_TIMESTAMP, 'robot123', 'c51ce410c124a10e0db5e4b97fc2af39', 2);
            """.formatted(USER_TABLE_NAME);
    ...
    public void checkTable(String databaseDriver, String databaseUrl, String databaseUsername, String databasePassword) throws ClassNotFoundException, SQLException {
        ...
        if (!dbMetaData.getTables(null, null, FORUM_TABLE_NAME, null).next()) createForumTable(connection);
        ...
        if (!dbMetaData.getTables(null, null, COMMENT_TABLE_NAME, null).next()) createCommentTable(connection);
    }
    ...
    public void createCommentTable(Connection connection) throws SQLException {
        connection.createStatement().execute(CREATE_COMMENT_TABLE_SQL);
    }

    public void createForumTable(Connection connection) throws SQLException {
        connection.createStatement().execute(CREATE_FORUM_TABLE_SQL);
    }
