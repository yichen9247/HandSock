...
public class OU_UploadService {
    ...
    public Map<String, Object> clientUploadFile(HttpHeaders headers, @RequestParam("file") MultipartFile file, String type) throws IOException {
        ...
        if (!isValidRequest(uidHeader, gidHeader, tokenHeader)) return new HandleResults().handleResultByCode(400, null, "禁止访问");

        if (!cacheService.validRedisUploadCache(Long.parseLong(gidHeader.get(0)))) {
            return new HandleResults().handleResultByCode(402, null, "操作频率过快");
        ...
        if (Objects.equals(type, "file") && !or_chanService.getChanOpenStatus(Long.parseLong(gidHeader.get(0)))) {
            return new HandleResults().handleResultByCode(402, null, "该频道未开启");
        }
        ...
        if (Objects.equals(type, "file") && ou_systemService.getSystemKeyStatus("taboo") && !op_userService.getUserAdminStatusByUid(Long.parseLong(uidHeader.get(0)))) {
            return new HandleResults().handleResultByCode(402, null, "全频禁言开启中");
        }

        if (!ou_systemService.getSystemKeyStatus("upload") && !op_userService.getUserAdminStatusByUid(Long.parseLong(uidHeader.get(0)))) {
            return new HandleResults().handleResultByCode(402, null, "上传权限暂未开放");
        }
        ...
        if (Objects.equals(type, "file") && or_userService.getUserTabooStatus(Long.parseLong(uidHeader.get(0))) && !op_userService.getUserAdminStatusByUid(Long.parseLong(uidHeader.get(0)))) {
            return new HandleResults().handleResultByCode(402, null, "你正在被禁言中");
        }
        ...
        try {
            ...
            new OU_UploadManage().insertUploadFile(ou_uploadModel, uid, fileName, filePath, time);
            if (ou_uploadDao.insert(ou_uploadModel) > 0) {
                new ConsolePrints().printInfoLogV2("New upload file " + uid + " " + filePath);
                return new HandleResults().handleResultByCode(200, new HashMap<>(){{
                    put("path", filePath);
                }}, "上传成功");
            } else return new HandleResults().handleResultByCode(400, null, "上传失败");
        } catch (IOException e) {
