...
public class OR_ChanService {
    ...
    public Map<String, Object> deleteChan(Long gid) {
        if (gid == 0) return new HandleResults().handleResultByCode(409, null, "主频道不可操作");
        try {
            List<OR_ChatModel> or_chatModels = or_chatDao.selectList(new QueryWrapper<OR_ChatModel>().eq("gid", gid));
            if (or_chatModels.isEmpty()) {
                if (or_chanDao.deleteById(gid) > 0) return new HandleResults().handleResultByCode(200, null, "删除成功");
            } else {
                if (or_chanDao.deleteById(gid) > 0 && or_chatDao.delete(new QueryWrapper<OR_ChatModel>().eq("gid", gid)) > 0) return new HandleResults().handleResultByCode(200, null, "删除成功");
            }
    ...
    public Map<String, Object> updateChan(Long gid, String name, String avatar, String notice) {
        try {
            if (or_chanDao.selectOne(new QueryWrapper<OR_ChanModel>().eq("gid", gid)) == null) return new HandleResults().handleResultByCode(409, null, "频道不存在");
    ...
    public Map<String, Object> createChan(Long gid, String name, String avatar, String notice) {
        try {
            if (or_chanDao.selectOne(new QueryWrapper<OR_ChanModel>().eq("gid", gid)) != null) return new HandleResults().handleResultByCode(409, null, "频道已存在");
            OR_ChanModel or_chanModel = new OR_ChanModel();
            new OR_ChanManage().setChan(or_chanModel, gid, name, avatar, notice);
            if (or_chanDao.insert(or_chanModel) > 0) {
                return new HandleResults().handleResultByCode(200, null, "创建成功");
            } else return new HandleResults().handleResultByCode(400, null, "创建失败");
        } catch (Exception e) {
            new ConsolePrints().printErrorLog(e);
            return new HandleResults().handleResultByCode(500, null, "服务端异常");
        }
