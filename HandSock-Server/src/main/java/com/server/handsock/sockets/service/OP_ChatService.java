...
public class OP_ChatService {
    ...
    public Map<String, Object> insertChatMessage(String type, long uid, long gid, String address, String content) {
        try {
            ...
            if (opChatDao.insert(OPChatModel) > 0) {
                return new HandleResults().handleResultByCode(200, result, "发送成功");
            } else return new HandleResults().handleResultByCode(400, null, "发送失败");
        } catch (Exception e) {
            new ConsolePrints().printErrorLog(e);
    ...
    public Map<String, Object> searchAllChatHistory(Long gid) {
        try {
            ...
            return new HandleResults().handleResultByCode(200, opChatDao.selectList(wrapper.eq("gid", gid)), "获取成功");
        } catch (Exception e) {
            new ConsolePrints().printErrorLog(e);
            return new HandleResults().handleResultByCode(500, null, "服务端异常");
        }
