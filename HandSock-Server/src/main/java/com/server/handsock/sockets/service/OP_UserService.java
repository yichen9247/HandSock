...
import com.server.handsock.sockets.dao.OD_UerDao;
...
public class OP_UserService {
    ...
    private final OD_UerDao ODUerDao;
    private final TokenService tokenService;

    @Autowired
    public OP_UserService(OR_UserDao or_userDao, OD_UerDao ODUerDao, TokenService tokenService) {
        this.or_userDao = or_userDao;
        this.ODUerDao = ODUerDao;
        this.tokenService = tokenService;
    }

    public OP_UserModel queryUserInfo(long uid) {
        return ODUerDao.selectById(uid);
    }
    ...
    public Boolean getUserAdminStatus(SocketIOClient client) {
        try {
            ...
            OP_UserModel selectResult = ODUerDao.selectOne(new QueryWrapper<OP_UserModel>().eq("uid", uid));
    ...
    public Map<String, Object> queryAllUser() {
        try {
            return handleResult(200, ODUerDao.selectList(null), "获取成功");
        } catch (Exception e) {
            new ConsolePrints().printErrorLog(e);
    ...
    public Map<String, Object> loginUser(String username, String password, String address) {
        ...
        if (selectResult == null || !selectResult.getPassword().equals(new MD5Encoder().toMD5(password))) {
            ...
            return handleResult(200, Map.of("token", token, "userinfo", ODUerDao.selectOne(new QueryWrapper<OP_UserModel>().eq("username", username))), "登录成功");
        }
    }
