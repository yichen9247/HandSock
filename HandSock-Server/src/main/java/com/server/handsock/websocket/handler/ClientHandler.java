    ...
    public void handleClientInit(SocketIOClient client, Map<String, Object> data, AckRequest ackRequest) {
        ...
        try {
            if (clientService.validClientToken(client)) {
                ...
                ackRequest.sendAckData(new HandleResults().handleResultByCode(200, result, "获取成功"));
            } else ackRequest.sendAckData(new HandleResults().handleResultByCode(403, null, "请重新登录"));
        } catch (Exception e) {
