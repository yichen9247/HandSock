...
public class AdminHandler {
    ...
    public void handleAdminRequest(SocketIOClient client, Map<String, Object> data, AckRequest ackSender, String action, String uidKey, String valueKey) {
        if (isValidClient(client)) {
            ackSender.sendAckData(new HandleResults().handleResultByCode(403, null, "禁止访问"));
            return;
        }

        try {
            Object value = getClientData(data, uidKey);
            Object valueData = valueKey != null ? getClientData(data, valueKey) : null;
            ...
            switch (action) {
                ...
                case "deleteChan":
                    ...
                    break;
                case "deleteChat":
                    sendAckData(ackSender, deleteChat(data));
                    break;
                case "deleteUpload":
                ...
                case "updateUserTabooStatus":
                    ...
                    break;
                default:
                    sendAckData(ackSender, new HandleResults().handleResultByCode(400, null, "未知操作"));
                    break;
            }
        } catch (Exception e) {
    ...
    private void handleException(Exception e, AckRequest ackSender) {
        ...
        ackSender.sendAckData(new HandleResults().handleResultByCode(500, null, "内部服务器错误"));
    }
    ...
    private Object deleteChat(Map<String, Object> data) {
        ClientService clientService = new ClientService(service);
        OR_ChatService orChatService = (OR_ChatService) service.get("or_chatService");
        return orChatService.deleteChat(clientService.getClientData(data, "sid"));
    }
